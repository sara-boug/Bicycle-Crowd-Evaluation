...
class Question2(Question): 
    ...
    def get_annotation_trend(self)->None: 
        ...
        cantsolve_count = cantsolve_count_dict["true"] +  cantsolve_count_dict["True"] 
        
        corrupt_count_dict = self.dataframe["corrupt_data"].value_counts().to_dict() 
        corrupt_count = corrupt_count_dict["true"] +  corrupt_count_dict["True"] 
        ...
        for state, frame in self.dataframe.groupby(["user_id","cant_solve", "corrupt_data" ]): 
            cant_solve =len( frame[(frame["cant_solve"]=="true") |(frame["cant_solve"]=="True")] )
            corrupt =len( frame[(frame["corrupt_data"]=="true") |(frame["corrupt_data"]=="True")] )
    ...
    def __process_df(self,df:pd.DataFrame)->None:
        ...
        corrupt_df = df[(df["corrupt_data"]=="true") |(df["corrupt_data"]=="True")]
